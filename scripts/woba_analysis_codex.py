...
def compute_metric_set(
    ...
    ba, slg, hits = compute_ba_slg_matrices(subset, x_edges, z_edges)
    return {
        "woba": compute_woba_matrix(subset, x_edges, z_edges),
        "ba": ba,
        "slg": slg,
        "count": compute_ab_count_matrix(subset, x_edges, z_edges),
        "hits": hits,
    }
...
def _plot_count_grid(
    ...
    *,
    ...
    show_fraction = hits is not None and np.isfinite(hits).any()

    for zi, zc in enumerate(z_centers):
        for xi, xc in enumerate(x_centers):
            ab_val = int(counts[zi, xi])
            if ab_val <= 0:
                continue
            color = "white" if (vmax > 0 and ab_val > vmax * 0.45) else "black"
            if show_fraction:
                hit_val = int(hits[zi, xi]) if np.isfinite(hits[zi, xi]) else 0
                ax.text(xc, zc - 0.05, f"{hit_val}/{ab_val}", ha="center", va="center", fontsize=3.5, color=color, fontweight="bold")
                ax.text(xc, zc + 0.08, str(ab_val), ha="center", va="center", fontsize=2.5, color=color)
            else:
                ax.text(xc, zc, str(ab_val), ha="center", va="center", fontsize=4, color=color)

    cbar = fig.colorbar(mesh, ax=ax, pad=0.02)
    cbar.set_label("Plate Appearances" if show_fraction else "Number of pitches")
        ...
        for key, metrics in matrices.items():
            ...
            if metrics["count"].sum() > 0:
                _plot_count_grid(
                    ...
                    title=f"{title_prefix} Plate Appearances ({config['label']})",
                    figsize=config["single_figsize"],
